...
const firstLetterForAvatar = authStore.userInfo.email[0].toUpperCase()
  ...
  <div class="flex flex-col w-full">
    ...
    <div class="container mx-auto px-4 -mt-24">
      <div class="flex flex-col md:flex-row items-center md:items-start space-y-4 md:space-y-0 md:space-x-6">
        <!-- Avatar -->
        <div class="w-40 h-40 rounded-full overflow-hidden border-4 border-white bg-white shadow-lg">
          <!-- Replace with actual avatar image if available -->
          <div class="w-full h-full bg-gray-300 flex items-center justify-center text-gray-600 text-4xl font-bold">
            {{ firstLetterForAvatar }}
          </div>
        </div>
      ...
      <div class="flex gap-7 items-center mt-8">
        ...
        <div class="flex gap-4">
          <button @click="switchTab(EditProfile)" class="text-sm font-medium text-green-600 hover:text-black">Редактировать профиль</button>
          <button @click="switchTab(QuestionCardsUser)" class="text-sm font-medium text-green-600 hover:text-black">Мои опросы</button>
        </div>
      </div>
      
      <KeepAlive>
        <component :is="currentTab" class="mt-4"></component>
      </KeepAlive>
    </div>
  </div>
</template>
